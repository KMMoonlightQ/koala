...
无需安装 Rust。首次运行 `koala` 自动创建 `~/.koala/config.toml`，并在界面内引导填写服务地址、API Key 和模型名。填写后按 Ctrl+S 保存并进入对话；已有完整配置会直接打开对话。
...
`[llm]` 段中添加（推理档位按服务商能力填写；上下文容量通常自动识别）：
...
`reasoning_effort` 参数。模型容量优先根据服务商端点和精确模型 ID 从 [Models.dev](https://models.dev/) 目录解析；
`context_window` 是自动识别失败时使用的正整数兜底值，单位为 tokens。解析后的容量同时用于自动压缩和状态栏占用率。
启动直接使用本地缓存或内置快照（2026-09-22）；缓存超过 24 小时后后台刷新，下一次启动使用新目录，网络失败不影响启动。
目录缓存位于系统缓存目录的 `koala/models-context.json`，查询不发送 API 密钥或自定义请求头。
精确端点匹配优先，其次解析带服务商的模型 ID；无法区分服务商且容量存在歧义时使用手动兜底，不模糊匹配相似名称。
...
- `Ctrl+C`：有文本选区时复制到系统剪贴板；无选区时，执行中中断整轮，空闲时清空输入。
- `Ctrl+D`：输入为空时退出；有草稿时交给输入框处理，不直接退出。
- 鼠标左键拖选：输入区和对话输出区均支持选区高亮、中文和跨行选择。输入区单击可定位光标，选中后输入或删除会替换或删除选区。输出选区随内容滚动，可按住左键配合滚轮扩展选区；调整窗口大小或切换视图会清除输出选区。
- 选中后用 `Ctrl+C` 复制，`Esc` 先取消选区；也支持终端能够透传的 `Ctrl+Shift+C` / `Cmd+C`。复制失败会显示提示并保留选区；剪贴板位于运行 Koala 的电脑上。
- 鼠标滚轮或 `PageUp` / `PageDown`（Mac 紧凑键盘可用 `Fn+↑` / `Fn+↓`）：滚动对话内容，不召回输入历史。上翻后新内容不会抢走阅读位置，翻到底部后恢复跟随。
- `Ctrl+End`：回到底部并恢复跟随。
...
由后端同步到状态栏；用量来自流式响应的 `usage`，不会累加历次请求。首次请求前、压缩或恢复会话后、切换模型后，以及接口未返回用量时显示 `--%`；无法自动识别且未配置兜底容量时隐藏百分比。
...
`agent.compact_threshold` 是上下文占用百分比，默认 `75`（75%），合法范围为 `1..100`。
例如 `deepseek-v4-flash` 在 DeepSeek 官方端点的容量为 1,000,000 tokens，默认压缩预算为 750,000 tokens。
请求前估算系统提示词、历史、工具定义、扩展/后台上下文和图片的 token 数，加上 4,096 tokens 回复预留；达到预算即触发压缩。
文本使用保守启发式估算（ASCII 约 3 字节/token，非 ASCII 按字符的 UTF-8 长度每 2 字节向上取整），图片按尺寸估算，不计 Base64 传输长度；
这不是模型专用 tokenizer 的精确计数，状态栏仍使用接口返回的真实 usage。完全未知且未填容量时，预算按保守的 16,384 tokens 窗口计算。
切换模型会同步切换容量与预算，主 Agent、子 Agent、临时对话及摘要请求共用预算规则。
旧配置如 `compact_threshold = 40000` 会明确报错，需迁移为 `compact_threshold = 75`。
